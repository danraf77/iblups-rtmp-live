...
check_health() {
    ...
    if ! docker ps | grep -q 'srs-ingest'; then return 1; fi
    if ! docker ps | grep -q 'backend-go'; then return 1; fi
...
if [ "$FAILS" -eq 1 ]; then
    send_email "WARNING (1): IBLUPS Ingest" "Falla detectada. Reintentando."
elif [ "$FAILS" -eq 2 ]; then
    send_email "ALERT (2): IBLUPS Ingest" "Segunda falla detectada."
elif [ "$FAILS" -ge 3 ]; then
    send_email "CRITICAL (3): IBLUPS Ingest" "Reiniciando el stack completo (docker compose down/up)."
    cd $BASE_DIR || exit 1
    docker compose down
    docker compose up -d
    rm "$FAIL_COUNT_FILE"
